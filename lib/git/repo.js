...
Repo.prototype.init = function(cb) {
  ...
  for (var branch_name in this.branches) {
    ...
    branch.init(function(err) {
      //if (err) return wrapped_cb(err); Disabled to avoid git2consul from closing if a branch is removed from the git repo
      if (err) {
        logger.error('Failed to init branch due to %s', err)
      }
